...
namespace Bluetooth_Tutorial
{
    public partial class SecureYourDataForm : Form
    {
        ...
        private bool isDecryptionStarted = false;
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    catch (Exception ex)
                    {
                        if (ticker != null)
                            ticker.Dispose();
                        Util.Logger().LogError("Disconnected", MSG_SERVER_RESTERTED, ex.Message+"--"+ex.Source);
        ...
        {
            isReplyTimeOut = true;
            Util.Logger().LogError(MSG_CHALLENGE_REPLY_TIMEOUT,string.Empty, string.Empty);
            mStream.Close();            
        }
