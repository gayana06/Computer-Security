...
namespace Bluetooth_Tutorial
{
    public class Crypto
    {
        ...
        {
            var keyBytes = new byte[AES_KEY_SIZE/8];
            var ivKeyBytes = new byte[AES_IV_SIZE/8];
            var secretKeyBytes = Encoding.UTF8.GetBytes(secretKey);
            Array.Copy(secretKeyBytes, keyBytes, Math.Min(keyBytes.Length, secretKeyBytes.Length));
            Array.Copy(secretKeyBytes, ivKeyBytes, Math.Min(ivKeyBytes.Length, secretKeyBytes.Length));
